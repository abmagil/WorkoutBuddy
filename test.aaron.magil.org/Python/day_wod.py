        ...
        for regex in re_list:
            ...
            if match:
                if regex == Named:
                   ...
                   print 'hit comment' #debug
                   return False
       ##If we didn't match one of our current regexes, we go to the next line and see if that matches.  
       ##Falls down when we have no descriptor, just goes right into the exercises (among other ways)
        else:  
...
def print_exercises(p):
    ...
    for exercise in exercises: ##exercise is now the whole exercise string, pull out more info
        ...
        exercise = exercise.strip().capitalize()
...
day = soup.find('div','dateheader')
find_exercise_type(content)

print "To commit this day's exercises, click here: 'http://aaron.magil.org/Python/commit_day_wod.py'"
