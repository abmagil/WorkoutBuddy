...
def print_exercises(p):
    ...
    for exercise in exercises: ##exercise is now the whole exercise string, pull out more info
        ...
        query = "INSERT into Exercise_List(exercise_name,exercise_type,date_added,defining_user) VALUES('"+ exercise +"','weight','"+ exercise_date +"','system') on duplicate key update defining_user='system';"
        cursor.execute(query)
        conn.commit()
        for feature in ['reps','time','difficulty']:
            value = str(eval(feature)).strip()
            if value != "":
                exer_tag += "<span>"+ value +"</span>"
        exer_tag += "</span><br/>"
        print exer_tag
    return True
